...
function startupCode() {
    ...
    $('a#check_in_family').on('click', function(e) {
                              ...
                              var print_tags = $('input#print_tags').val();
                              var print_parent_tags = $('input#print_parent_tags').val();
                              var print_additional_tags = $('input#print_additional_tags').val();
                              var print_check_in = $('input#print_check_in').val();
                              var parent_tag = '';
                              
                              $.each(selected_rows, function(index, value) {
                                     var person_id = $(value).data('person_id');
                                     if ( $(this).hasClass( 'previously_checked_in' ) == false ) {
                              ...
                              $.ajax({
                                     ...
                                     $.each($(selected_people_ids), function(index, person_id) {
                                            ...
                                            //reset child_or_parent for child tag
                                            child_or_parent = [];
                                            
                                            //get the person's tag list
                                            person_tag_list = $('a[data-person_id="' + person_id + '"]').attr('data-person_tag_list');
                                            
                                            child_or_parent = get_print_child_or_parent(print_tags, print_parent_tags, print_additional_tags, person_tag_list, print_check_in,'family_child');
                                            if (child_or_parent['parent']) {
                                            //if a child requires a parent tag
                                            parent_tag = person_tag_list;
                                            child_or_parent['parent'] = 0;
                                            }
                                            
                                            
                                            print_tag(person_id, instance_id_to_pass, child_or_parent, selected_checkout); // print nametag for child 'child'
                                            });
                                     
                                     //reset child_or_parent for parent tag
                                     child_or_parent = [];
                                     child_or_parent = get_print_child_or_parent(print_tags, print_parent_tags, print_additional_tags, parent_tag, print_check_in,'family_parent');
                                     
                                     print_tag(selected_people_ids, instance_id_to_pass, child_or_parent, selected_checkout); // print nametag for parent
                                     
                                     if(mode == 'search') {
                                     clear_screen_timer = setTimeout(function() { if(is_typing == false) { $('input#filter_people').val(''); $('input#filter_people').focus(); } }, 5000);
                                     }
    ...
    $('table#people_list').on('click', 'a.checkin', function(event) {
                              ...
                              var action = $(this).data('action');
                              var person_tag_list = $(this).attr('data-person_tag_list');
                              var link_container = $(this).closest('a');
                              ...
                              load_checkin_people_from_instance_id_request = $.ajax({
                                                                                    ...
                                                                                    // mark previously checked in family members
                                                                                    mark_previously_checked_in_family_members();
                              ...
                              child_or_parent = get_print_child_or_parent(print_tags, print_parent_tags, print_additional_tags, person_tag_list, print_check_in, 'child');
                              print_tag(person_id, instance_id, child_or_parent , checkout);
...
function  mark_previously_checked_in_family_members() {
    
    $('div#family_checkin_container').find('a').each(function() {
                                                     if ($(this).hasClass('in') == true) { $(this).addClass('previously_checked_in') }
                                                     });
...
function print_tag(person_id, instance_id, child_or_parent, checkout) {
    ...
    $.ajax({
           ...
           update_link_container_style(person_id, 'in_override', checkout, '', $('table#people_list').hasClass('initialized') );
           // create child label
           if(child_or_parent['both'] == 1 || child_or_parent['child'] == 1) {
           //console.log('printing child: ' + person_id);
           ...
           if(child_or_parent['both'] == 1 || child_or_parent['parent'] == 1) {
           //console.log('printing parent: ' + person_id);
           ...
           if($('input#print_additional').val() == '1') {
           if ( (child_or_parent['no_tags'] == 1) || (child_or_parent['additional'] == 1) ) {
           //console.log('printing additional:' + person_id);
           if(nametag_fields.child_image && nametag_fields.child_image != "0") { child_label_xml = child_image_label_xml; } else { child_label_xml = child_text_label_xml; } // determine if text or image
           child_label = dymo.label.framework.openLabelXml(child_label_xml);
           child_label = assign_substitutions(child_label, person);
           print(child_label);
...
function update_printer_status(instance_id, oid) {
    ...
    if($('input#print').val() > 0) {
        ...
        if (!('none' in all_printers)) {
            all_printers['none'] = "None";
            // add none as option
            $('select#printer_list')
...
function list_local_printers() {
    ...
    $.each(printers, function(index, printer) {
           ...
           if(printer_id in all_printers) {
           return false;
           }
           
           all_printers[printer_id] = printer_name;
           ...
           // console.log('Adding ' + printer_name + ' to list of printers.');
    ...
    if(printer_found) {
        ...
        try { print_station_error('DYMO Software Not Found', 'We were not able to detect the DYMO Label Software as being installed on this computer.  Please use our <a href="../../../../../events/printer">Test Print</a> page to diagnose this issue.'); } catch(err) { }
...
function load_instance_by_instance_id(instance_id, oid) {
    ...
    $.ajax({
           ...
           $.ajax({
                  ...
                  $('input#print_tags').val(result.check_in_print_tags);
                  $('input#print_parent_tags').val(result.check_in_print_parent_tags);
                  $('input#print_additional_tags').val(result.check_in_print_additional_tags);
                  $('input#print_additional').val(result.check_in_print_additional_name_tag);
                  $('input#print_check_in').val(result.check_in);
                  $('input#show_tag_name_on_check_in').val(result.show_tag_name_on_check_in);
                  $('input#event_tag_list').val(JSON.stringify(result.event_tag_list));
...
function update_people_list(result, hide_alphabet, table_id_to_populate) {
    ...
    if(people_in_list > 500) {
        // Delay is needed to ensure that $.mobile.activePage is fully loaded
        setTimeout(function() {
    ...
    if ( $('input#print_check_in').val() == "tags" ) {
        var event_tag_list = JSON.parse($('input#event_tag_list').val());
    }
    
    if( people_in_list > 20 && $('input#print_check_in').val() == "everyone" && !show_more_link && countProperties(filter_by_tag_ids_json) < 3 && mode == "search") {
        //removes one person so that show_more link shows.
        result.pop();
        show_more_link = true;
    } else {
    ...
    $.each(result, function() {
           ...
           if (!this.tag_list) { this.tag_list = ''; }
           
           var person_tag_list = '';
           if ( ($('input#print_check_in').val() == "tags" ) && ($('input#show_tag_name_on_check_in').val() == '1') ){
           for (var key in event_tag_list ) {
           if ( (jQuery.inArray(event_tag_list[key]['tag_id'], this.tag_list) > -1) ) {
           if (!person_tag_list) { person_tag_list = event_tag_list[key]['name']; }
           else { person_tag_list += ", " + event_tag_list[key]['name']; }
           }
           }
           }
           
           var text_offset_class_if_tag_names_present = '';
           if(person_tag_list) { text_offset_class_if_tag_names_present = ' offset_for_tag_names'; }
           ...
           tr += '<div class="pull-left"><label class="control-label check_in_name" for="inputEmail"><div  class="check_in_block"><img src="'+ files_domain + this.profile_picture + '" class="profile"><div class="check_in_block_text' + text_offset_class_if_tag_names_present + '">'+this.last_name+', '+this.first_name+'<div class="checkin_row_tags">'+person_tag_list+'</div></div></div></label></div><div class="pull-right check_container">';
           ...
           if(this.check_out_datetime) {
           tr += '<a href="#" class="checkin checkout remove" data-action="remove" data-person_tag_list="'+this.tag_list+'" data-person_id="'+this.id+'" data-instance_id="'+instance_id+'"><span class="icon-stack"><i class="icon-sign-blank icon-stack-base"></i><i class="front_icon icon-remove icon-light"></i></span></a>';
           } else {
           tr += '<a href="#" class="checkin checkout in" data-action="out" data-person_tag_list="'+this.tag_list+'" data-person_id="'+this.id+'" data-instance_id="'+instance_id+'"><span class="icon-stack"><i class="icon-sign-blank icon-stack-base"></i><i class="front_icon icon-stop icon-light"></i></span></a>';
           }
           
           }
           
           if(!this.checked_in) { tr += '<a href="#" class="checkin checkout out" data-action="in" data-person_tag_list="'+this.tag_list+'" data-person_id="'+this.id+'" data-instance_id="'+instance_id+'"><span class="icon-stack"><i class="icon-sign-blank icon-stack-base"></i><i class="front_icon icon-play icon-light"></i></span></a>'; }
           ...
           if(this.checked_in) { tr += '<a href="#" class="checkin in" data-action="out" data-person_tag_list="'+this.tag_list+'" data-person_id="'+this.id+'" data-instance_id="'+instance_id+'"><span class="icon-stack"><i class="icon-sign-blank icon-stack-base"></i><i class="front_icon icon-ok icon-light"></i></span></a>'; }
           if(!this.checked_in) { tr += '<a href="#" class="checkin out" data-action="in" data-person_tag_list="'+this.tag_list+'" data-person_id="'+this.id+'" data-instance_id="'+instance_id+'"><span class="icon-stack"><i class="icon-sign-blank icon-stack-base"></i><i class="front_icon icon-ok icon-light"></i></span></a>'; }
    ...
    if (show_more_link) {
        ...
        tr += '<div id="show_more_people_checkin"><a href="#" class="btn btn-large" style="font-size:19px; padding:15px; background-color: #fff;background-image: none;">Show More</a></div>';
        
        tr += '</td><td class="hide"></td>';
        tr += '</tr>';
...
function assign_substitutions(label, person) {
    ...
    $.each(nametag_fields, function(placeholder, field) {
           ...
           label.setObjectText(placeholder, group_string);
           }
           } catch(err) { }
           break;
           ...
           var full_picture_path = files_domain + picture_path;
